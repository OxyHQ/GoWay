...
export interface GoWayHeadersLike {
  get(name: string): string | null;
}

/** The subset of a fetch `Response` the SDK reads. */
export interface GoWayFetchResponse {
  readonly status: number;
  readonly headers: GoWayHeadersLike;
  text(): Promise<string>;
}

/** The subset of `AbortSignal` the SDK reads. */
export interface GoWayAbortSignal {
...
export interface GoWayFetchInit {
  ...
  /** A JSON body, already serialised. Absent on every GET. */
  body?: string;
  /**
   * The request's `AbortSignal` (a {@link GoWayAbortSignal} at runtime).
   *
 ...
 */
export type GoWayFetch = (url: string, init: GoWayFetchInit) => Promise<GoWayFetchResponse>;

interface AbortControllerLike {
  readonly signal: GoWayAbortSignal;
  abort(): void;
}
 ...
 */
export function globalFetch(): GoWayFetch | undefined {
