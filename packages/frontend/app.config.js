...
//
// The three native icons are the MARK, never the wordmark. Two rows of bubble
...
//
// The web favicon is the wordmark, which is the user's own call and overrides
// that rule for the browser tab only. Rendered at 16, 32, 48 and 64 and looked
// at before committing: at 48 and above it reads as "GO WAY"; at 32 it is
// readable; at 16 it is a recognisable blue-and-white shape rather than legible
// words. That is the trade, and it is a reasonable one — 16px is the old
// non-retina tab and most browsers now ask for 32 or more.
//
...
//  - `favicon.png`       64, transparent — the WORDMARK, supplied by the user
//                        and downscaled from `favicon-source.png` (1000px),
//                        which is kept beside it so a future size can be cut
//                        from the original rather than from the 64px copy.
//                        This one is NOT generated by `scripts/build-brand.ts`,
//                        so `brand:check` does not police it; the two SVGs it
//                        does police are unaffected.
