...
A map is four separate downloads, and every one of them is now GoWay's. Loading
GoWay used to make the browser talk to `tiles.openfreemap.org` for tiles, glyphs
and the sprite, and to `goway.to` for the style document alone.
...
| tiles | `/map/tiles/{z}/{x}/{y}.pbf` | **built by us** with Planetiler, one PMTiles archive in R2, read by byte range by `worker/index.js` |

The tile row used to say *proxied*, and the paragraph under it used to say, in
so many words, that GoWay was not independent: if OpenFreeMap was down, GoWay's
map was down. [Building our own tiles](#the-tiles-planetiler-pmtiles-and-r2) is
what deleted that sentence.
...
bun run --cwd packages/frontend map:style:check --online   # also re-verify the FALLBACK upstream's schema
```
...
### The tiles — Planetiler, PMTiles and R2

```bash
bun run --cwd packages/frontend map:tiles --area=monaco      # the fast loop, ~90s
bun run --cwd packages/frontend map:tiles --area=cataluna     # the reference extract
bun run --cwd packages/frontend map:tiles --area=planet       # hours
bun run --cwd packages/frontend map:tiles:verify --archive=<path> --area=<area>
bun run --cwd packages/frontend map:tiles:upload --archive=<path> --key=basemap/planet-YYYYMMDD.pmtiles
```

`scripts/build-map-tiles.ts` runs **Planetiler** (pinned by version AND sha256)
over an OpenStreetMap extract and writes one **PMTiles** archive. The archive
goes to **Cloudflare R2**, and `worker/index.js` answers
`/map/tiles/{z}/{x}/{y}.pbf` by reading byte ranges out of it. There is no tile
server anywhere in GoWay.

**Why one file and not 350 million.** R2 is object storage: it hands back a
range of an object and nothing else. Either the planet is one object addressed
by `Range:` or it is one object per tile, and at ~$4.50 per million Class A
operations the upload alone would cost more than a decade of storage. PMTiles
is the format that makes the first option work — a header, a tree of
directories, a tile body, all reachable by offset.

**Why R2 and not S3.** Egress. R2 charges nothing for it; S3 charges $0.09/GB
out, which for a map is not a line item, it is the entire bill.

**Why the schema did not change.** `lib/map/style/schema.ts` records the
OpenMapTiles v3 vocabulary the style speaks, and Planetiler's basemap profile
*is* an OpenMapTiles v3 implementation — the same generator OpenFreeMap runs.
The first self-hosted build emitted the same sixteen `vector_layers` the record
already named, so the style document needed no change at all. What changed is
the confidence behind the `class` vocabularies: they can now be swept out of an
archive on disk instead of sampled through somebody else's CDN, and the first
sweep turned up values six cities of sampling had missed (`busway`, `raceway`,
`campsite`, `prison`, `hamlet`, and the `*_construction` road classes a comment
had always claimed were there).

#### The OpenStreetMap element id, which is the other half of the point

Planetiler sets each tile feature's id to **`osmId * 10 + sourceId`**, with
`sourceId` 1 for a node, 2 for a way and 3 for a relation. That is not a GoWay
invention and not a flag we turned on — it is Planetiler's
`feature_source_id_multiplier`, on by default, and it was already true of the
tiles GoWay was proxying.

It matters because it is the join GoWay needs and had been told it could not
have. An earlier measurement concluded that "OpenFreeMap's tile feature ids are
unrelated to OSM element ids", citing tile id `62887353` for La Boqueria and
`1889380012` for Museu Picasso, and de-duplicating a GoWay place against the
basemap's own label was worked around instead of solved. Divide by ten and read
the remainder:

| tile id | ÷10, remainder | OSM element | name on osm.org |
|---|---|---|---|
| `62887353` | 6288735 r3 | `relation/6288735` | Mercat de Sant Josep - La Boqueria |
| `1889380012` | 188938001 r2 | `way/188938001` | Museu Picasso |

The ids were never unrelated. What was wrong was the OSM ids on the GoWay side:
`places_sources` had `way/25336101` for La Boqueria (an unnamed way) and
`way/34633854` for Museu Picasso (which is the **Empire State Building**). The
join was impossible because one side of it was wrong, not because the tiles
withheld anything — and that is a Places data problem, still open, not a tiles
problem.

Owning the build is what makes the property **ours to keep** rather than a
default of somebody else's pipeline, so `map:tiles --verify` asserts it: every
feature in the `poi` layer must decode to an OSM element (measured: 137435 of
137435 across 63 sampled Catalonia tiles), and the two places above must be
found by their element ids.

#### Reproducibility

A planet build that happened once on a laptop is not infrastructure.
...
- **Every build writes a receipt** beside the archive: the OSM replication
  sequence and timestamp read back *out* of the finished file, the Planetiler
  version and git hash, the wall clock, the sampled feature counts and the id
  coverage. Two builds of the same replication sequence with the same pinned
  Planetiler are the same map.

Measured on the build machine (32 cores, 117 GB RAM): Monaco is 94 s and
0.42 MB; Catalonia is 70 s from a 270 MB extract and 186 MB out.

#### Verification, because a tile build fails silently

Planetiler exits 0 on a run that dropped a layer, stopped a zoom short or wrote
ids that decode to nothing. `map:tiles:verify` opens the artefact instead —
with `worker/pmtiles.js`, the **same reader the edge serves with**, so "the
archive is readable" and "GoWay can read the archive" are one claim — and
checks the container, the zoom range, the layer set in both directions, real
gunzipped tiles at every zoom from 0 to 14, and the id coverage. Nothing
replaces a live archive until that has passed; `map:tiles:upload` runs it first
and refuses on any problem.

#### Cutover, and why the key is versioned

A PMTiles archive is addressed by byte offset, so overwriting the object a
Worker is mid-request against does not serve a stale tile — it serves whatever
now lives at that offset, which is a directory decoded as a tile or a tile from
somewhere else on Earth. So:

1. upload to a **new** key (`basemap/planet-YYYYMMDD.pmtiles`);
2. point `MAP_TILE_ARCHIVE` in `wrangler.toml` at it and deploy;
3. delete the old object once the deploy is live.

Step 2 is also the rollback. The Worker's edge cache key names the archive key,
so the deploy invalidates every cached directory and tile in the world at the
instant it lands — otherwise the day-long tile TTL would keep serving the old
planet from every warm colo.

#### One-time human setup — **not yet done**

R2 is **not enabled** on the Oxy Cloudflare account (the API answers
`Please enable R2 through the Cloudflare Dashboard`), so the `[[r2_buckets]]`
block in `wrangler.toml` is commented out and the Worker falls back to proxying
`MAP_TILE_UPSTREAM`. `wrangler deploy` refuses a binding to a bucket that does
not exist, and a deploy that failed there would take `goway.to` down for a
change meant to be invisible.

1. Cloudflare dashboard → R2 → **Enable** (account-level opt-in; a human).
2. `wrangler r2 bucket create goway-map-tiles`
...
R2: **$0.015/GB-month**, **$4.50 per million Class A** (writes), **$0.36 per
million Class B** (reads), **$0 egress**. From the measured artefacts:
...
directory caching in `worker/index.js` exists to hold down: a warm tile request
is **one** Class B operation, because the header, the root directory and the
leaf directory are cached at the edge for a week and the archive key is in the
cache key. One million tile requests is **$0.36**, and the bytes that leave
Cloudflare cost nothing. The same planet on S3 would bill $0.09/GB of egress —
which for a map is not a line item, it is the whole bill.

Not included: the build machine (Oxy's own hardware, ~85 GB of planet PBF
downloaded once and shared with the routing engine) and Workers requests, which
GoWay already pays for serving the app.
...
**Building our own tiles did not weaken the obligation and did not change the
credit's content.** The credit follows the licence, not the hostname: OSM data
is ODbL and the OpenMapTiles schema is CC-BY, and both survive the move to R2
untouched — they are exactly the two credits Planetiler writes into the
archive's own metadata. What was dropped is the OpenFreeMap line, which was
courtesy for serving the bytes rather than a licence term, and GoWay serves its
own bytes now. `lib/map/provider.ts` → `ATTRIBUTION` is the one place to change
it, and `map:style:check` re-stamps it into both style documents.
