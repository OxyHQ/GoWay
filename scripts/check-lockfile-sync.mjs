...
// An UNTRACKED lockfile makes layer 2 vacuous, and silently: `git diff --quiet`
// exits 0 for a path git does not track, so a regenerated lockfile reads as
// "unchanged" and the gate reports sync over a file nothing is comparing. This
// is not hypothetical — a freshly scaffolded Oxy repository has every generated
// file untracked, which is exactly when somebody first runs this.
if (git(["ls-files", "--error-unmatch", "--", LOCKFILE], { allowFailure: true }).exitCode !== 0) {
  die(
    `${LOCKFILE} is not tracked by git, so a regenerated one cannot be compared against a committed one — ` +
      `\`git diff\` reports no change for an untracked path and this check would pass while measuring nothing. ` +
      `Run \`git add ${LOCKFILE}\` and commit it with its package.json.`,
  );
}
...
// TWO shapes, because this repository uses the second and the fleet uses the
// first. `<dir>/*` is a glob and every package.json under it is a workspace;
// anything else is a LITERAL path, which is how GoWay enumerates its four
// packages — and a literal path is where the interesting failure lives.
// `packages/reconstruction-worker` is deliberately NOT a Bun workspace (it is
// Python/CUDA), so a glob here would sweep it in the day it grows a
// package.json; the explicit list is load-bearing, and this check has to model
...
for (const pattern of declaredPatterns) {
  ...
  if (globMatch) {
    const [, workspaceDirectory] = globMatch;
  ...
  const literalPath = String(pattern).replace(/^\.\//, "").replace(/\/+$/, "");
  if (!/^[A-Za-z0-9._-]+(?:\/[A-Za-z0-9._-]+)*$/.test(literalPath)) {
    die(
      `Unsupported workspace pattern ${pattern}. This check models <directory>/* and literal paths; teach it the new shape rather than letting it stop comparing workspaces.`,
    );
  }
  // A declared workspace with no package.json is a manifest that describes a
  // package set the repository does not have. Skipping it would let the
  // comparison below shrink silently to whatever still exists.
  if (!(await Bun.file(join(repositoryRoot, literalPath, "package.json")).exists())) {
    die(
      `package.json declares the workspace ${literalPath}, which has no package.json. Remove it from \`workspaces\` or add the package.`,
    );
  }
  manifestWorkspacePaths.push(literalPath);
}
...
// bun folds `resolutions` INTO the overrides it records — one `"overrides"`
// block in bun.lock, whatever spelling the manifest used. This repository
// declares both (npm reads `overrides`, yarn reads `resolutions`, and the Oxy
// scaffolder writes the pair so a consumer of either tool gets the same pinned
// @oxy.so/* versions), so the two have to be reconciled before comparing.
//
// Declaring both and letting them DISAGREE is the failure this models: bun
// records one of them, the other is silently inert, and the pin a reviewer reads
// in `resolutions` is not the pin that resolved. That is a hard failure rather
// than a merge, because there is no correct answer to pick.
const declaredOverridesRaw = rootManifest.overrides ?? {};
const declaredResolutions = rootManifest.resolutions ?? {};
for (const name of [...new Set([...Object.keys(declaredOverridesRaw), ...Object.keys(declaredResolutions)])].sort()) {
  const inOverrides = declaredOverridesRaw[name];
  const inResolutions = declaredResolutions[name];
  if (inOverrides !== undefined && inResolutions !== undefined && inOverrides !== inResolutions) {
    staleRecords.push(
      `package.json pins ${name} to ${JSON.stringify(inOverrides)} in overrides but ${JSON.stringify(inResolutions)} in resolutions; bun records only one of them, so the other is inert.`,
    );
  }
}
const declaredOverrides = { ...declaredResolutions, ...declaredOverridesRaw };
